...
export default function DosenDashboard({ profile }: DosenDashboardProps) {
  ...
  const [newCourseProdi, setNewCourseProdi] = useState('S1 PGSD');
  const [newCourseAngkatan, setNewCourseAngkatan] = useState('2024');
  ...
  const handleCreateCourse = async (e: FormEvent) => {
    ...
    try {
      ...
      const courseData = {
        ...
        prodi: newCourseProdi,
        angkatan: newCourseAngkatan
      };

      const docRef = await addDoc(collection(db, 'courses'), courseData);
      
      // Auto-create Google Drive folder if integration is active
      try {
        ...
        if (driveCheck.ok) {
          ...
          if (driveData.connected) {
            await fetch('/api/drive/create-course-folder', {
              ...
              body: JSON.stringify({ courseId: docRef.id, courseNama: newCourseName })
            });
          }
        }
      } catch (driveErr) {
      ...
      setNewCourseName('');
      setNewCourseDesc('');
      setNewCourseProdi('S1 PGSD');
      setNewCourseAngkatan('2024');
              ...
              return (
                <>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Prodi:</span>
                        <select
                          value={prodiFilter}
                          onChange={(e) => setProdiFilter(e.target.value)}
                          className="block rounded-xl border border-slate-300 py-1.5 px-3 text-xs font-semibold text-slate-700 bg-white focus:border-blue-500 focus:outline-none"
                        >
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Angkatan:</span>
                        <select
                  ...
                  {loading ? (
                    ...
                    <div className="bg-white rounded-3xl border border-dashed border-slate-300 p-12 text-center">
                      <BookOpen className="h-12 w-12 mx-auto text-slate-400 mb-3" />
                      <h3 className="font-bold text-slate-800 text-lg">Tidak Ada Kelas yang Cocok</h3>
                      <p className="text-slate-500 text-sm mt-1 max-w-sm mx-auto">
                        Tidak ada kelas kuliah yang cocok dengan kriteria filter Program Studi atau Angkatan yang dipilih.
                      </p>
                    </div>
                  ) : (
                          ...
                          <div>
                            <div className="flex justify-between items-start mb-3">
                              ...
                              <div className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1 rounded-xl border border-slate-100 hover:bg-slate-100 transition" onClick={(e) => { e.stopPropagation(); handleCopyCode(course.kode); }}>
                                <span className="font-mono text-[10px] text-slate-600 font-bold tracking-wider">{course.kode}</span>
                          ...
                          <div className="flex items-center gap-2">
                            {sub.driveFileUrl && (
                              <a
                                ...
                                className="inline-flex items-center gap-1.5 text-[10px] font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-100 rounded-xl px-2.5 py-1.5 transition"
                              >
          ...
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-xl border border-slate-100 space-y-5 animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleCreateCourse} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Program Studi</label>
                  <select
                    value={newCourseProdi}
                    onChange={(e) => setNewCourseProdi(e.target.value)}
                    className="block w-full rounded-xl border border-slate-300 py-2 px-3 text-sm text-gray-900 focus:border-blue-500 bg-white"
                  >
                    <option value="S1 PGSD">S1 PGSD</option>
                    <option value="S1PMAT">S1PMAT</option>
                    <option value="S1PIPA">S1PIPA</option>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Angkatan</label>
                  <select
                    value={newCourseAngkatan}
                    onChange={(e) => setNewCourseAngkatan(e.target.value)}
                    className="block w-full rounded-xl border border-slate-300 py-2 px-3 text-sm text-gray-900 focus:border-blue-500 bg-white"
                  >
